...
public class Role {
    ...
    public static void add(Role role) {
        ...
        DbOperations.setDataOrDelete(query, "Role created successfully test etsts");
    }
    ...
    public static ArrayList<Role> Search(String txtString) {
        String ConTxt = "%" + txtString + "%";
        ArrayList<Role> arrayList = new ArrayList<>();
        try {
            ResultSet rs = DbOperations.getData("select  * from role where RoleName like '" + ConTxt + "'");
            while (rs.next()) {
                Role role = new Role();
                role.setRoleId(rs.getInt("RoleId"));
                role.setRoleName(rs.getString("RoleName"));
