...
public class Orders {
    ...
    public Float GetAvg(String StartDate, String EndDate, String orStatus) {
        Float AvrData = 0f;
        try {
            String sql = "select (select sum(Total) from Orders where OStatus='" + orStatus + "' And date between '" + StartDate + "' and '" + EndDate + "')/(select count(t.PhoneNumber) "
                    + "from (select distinct PhoneNumber from Orders where OStatus='" + orStatus + "') t) AvgPrice";
            ResultSet rs = DbOperations.getData(sql);
            while (rs.next()) {
    ...
    public void HighPrice(String StartDate, String EndDate, String orStatus) {
        try {
            String sql = "select PhoneNumber,sum(Total) Hprice from Orders where  OStatus='"+orStatus+"' AND date between '" + StartDate + "' and '" + EndDate + "' group by PhoneNumber  order by sum(Total) desc limit 1";
            ResultSet rs = DbOperations.getData(sql);
            while (rs.next()) {
    ...
    public void MaxItem(String StartDate, String EndDate,String orStatus) {
        try {
            String sql = "select FoodItem,sum(FoodQuantity)TotalQuantity from OrderDetail d left join Orders O on d.OrderId = O.OrderId where OStatus='"+orStatus+"' AND OrderDate between '" + StartDate + "' and '" + EndDate + "' "
                    + "group by FoodItem  order by sum(FoodQuantity) desc limit 1";
            ResultSet rs = DbOperations.getData(sql);
            while (rs.next()) {
    ...
    public void MinItem(String StartDate, String EndDate,String orStatus) {
        try {
            ...
            "where o.OStatus='Fulfilled' AND OrderDate between '" + StartDate + "' and '" + EndDate + "' group by FoodItem  order by sum(FoodQuantity) asc limit 1";
            ResultSet rs = DbOperations.getData(sql);
            while (rs.next()) {
    ...
    public static ArrayList<Orders> ReportOut(String FDate, String TDate,String orStatus) {
        ArrayList<Orders> arraylist = new ArrayList<>();
        try {
            String sql = "select * from Orders where  date  between '" + FDate + "' and '" + TDate + "'And OStatus='"+orStatus+"'";
            ResultSet rs = DbOperations.getData(sql);
            while (rs.next()) {
